...
export class PageService {
  ...
  handleDataOfPage(name_component:string,data:Array<any>){
    let dataFinal ={};
    let type_comp = {};
    let dataSorted = data.sort((ele1,ele2)=>{
        let a = +ele1.location.orderly;
        let b = +ele2.location.orderly;
        if(a >= b){
          return 1;
        }
        if( a <= b){
    ...
    type_comp["type".trim()] = name_component;
    dataFinal["data".trim()] = dataSorted;
    dataFinal = Object.assign(type_comp,dataFinal);
    return dataFinal;
  }
}